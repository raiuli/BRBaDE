...
%     end    
    ...
    load(s,'data');
    pso_data=data;
    s=strcat(fname_v(i),'_LSHADE_EpSin.mat');
    load(s,'data');
    LSHADE_EpSin(:,1)=data;
    LSHADE_EpSin(:,2)=1:1:length(data);
%      LSHADE_EpSin=brbinsde_data;
%      LSHADE_EpSin(:,9)=brbinsde_data(:,9)-3.0;
%      ss=sprintf('%s_LSHADE_EpSin.mat',fname_v(i));
%    
%     save(ss,'data');
    ...
    %plot(brbinsde_data(:,10),brbinsde_data(:,9),'-k',fade_data(:,10),fade_data(:,9),'--r',de_data(:,10),de_data(:,9),':b',pso_data(:,10),pso_data(:,9),'-.g'),xlabel('iterantions'), ylabel('f(x)'),...
     plot(brbinsde_data(:,10),brbinsde_data(:,9),'-k',...
          ...
          LSHADE_EpSin(:,2),LSHADE_EpSin(:,1),':b',...
          'LineWidth',2) 
          xlabel('iterantions'), ylabel('f(x)')
         legend('BRBADE', 'FADE','LSHADE_EpSin')
         title(title_v(i))
    ...
    saveas(gcf,s)
    s=sprintf('%s,%5.5f,%5.5f,%5.5f,%5.5f--%5.5f,%5.5f,%5.5f,%5.5f',title_v(i),brbinsde_data(size(brbinsde_data,1),9),fade_data(size(fade_data,1),9),de_data(size(de_data,1),9),pso_data(size(pso_data,1),9)...
        ,brbinsde_data(size(brbinsde_data,1),10),fade_data(size(fade_data,1),10),de_data(size(de_data,1),10),pso_data(size(pso_data,1),10));
    disp(s);
end
