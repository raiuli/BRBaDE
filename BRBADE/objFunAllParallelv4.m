function f=objFunAllParallelv4(x1,brbConfigdata)
%tic
%fprintf('o');
...
tic
parfor data_id=1:sizeOfData
    matchingDegrees(data_id,:)=calMatchingDegree(transformedRefValM(data_id  ,:,1:size(transformedRefValM,3)),attrWeight);
end
toc
tic
...
ruleweights=repmat(ruleweight',sizeOfData,1);
z=sum(ruleweights.*matchingDegrees,2);
activationWeights=(ruleweights.*matchingDegrees)./z;
...
%      end
    ...
    ruleweight=ones(numOfRuleWeight,1);
    ruleweight=x1(numOfAttrWeight+1:numOfAttrWeight+numOfRuleWeight)';
    %c=repmat(ruleweight,1,sizeOfData);
%     j=numOfAttrWeight ;
%     for i=1:numOfRuleWeight
%         j=j+1;
%         ruleweight(i)=x1(j);
