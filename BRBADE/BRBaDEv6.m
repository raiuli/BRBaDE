...
for i=1:NP
    ...
    t=zeros(1,D);
    %generation of attAttrWeight and RuleWeight 
    ...
    t(numOfAttrWeight+numOfRuleWeight+1:numOfVariables-numOfconRefval-numOfAntecedentsRefVals)=z;
    %generation of conRefval
    t(numOfVariables-numOfconRefval-numOfAntecedentsRefVals+1:numOfVariables-numOfAntecedentsRefVals) = XVmin(numOfVariables-numOfconRefval-numOfAntecedentsRefVals+1:numOfVariables-numOfAntecedentsRefVals) + ...
        rand(1,numOfconRefval).*(XVmax(numOfVariables-numOfconRefval+1:numOfVariables)...
        - XVmin(numOfVariables-numOfconRefval-numOfAntecedentsRefVals+1:numOfVariables-numOfAntecedentsRefVals));
    t(numOfVariables-numOfconRefval-numOfAntecedentsRefVals+1:numOfVariables-numOfAntecedentsRefVals) = sort(t(numOfVariables-numOfconRefval-numOfAntecedentsRefVals+1:numOfVariables-numOfAntecedentsRefVals),'descend');
    %generation of Antecedent attributes utility values
    beginMarker=numOfAttrWeight+numOfRuleWeight+numOfbeliefDegrees+numOfconRefval+1;
    t(beginMarker:numOfVariables) = XVmin(beginMarker:numOfVariables)...
        + rand(1,numOfAntecedentsRefVals).*(XVmax(beginMarker:numOfVariables)...
        - XVmin(beginMarker:numOfVariables));
    %offset=numOfAntecedentsRefVals/numOfAttrWeight;
    %t(beginMarker:beginMarker+offset-1)=sort(t(beginMarker:beginMarker+offset-1),'descend');
    %beginMarker=beginMarker+offset;
    %t(beginMarker:beginMarker+offset-1)=sort(t(beginMarker:beginMarker+offset-1),'descend');
    pop(i,:)=t;
end
...
oldCR=[];
oldBestval=[];
f_history=zeros(NP,itermax);
popHistory=zeros(NP,D,itermax);
while ((iter < itermax) & (bestval > VTR))
    ...
    if (iter>1 )
        ...
        oldBestval=horzcat(oldBestval,round(bestval,6));
        if iter>=1000
            if (length(oldBestval)>1002)&& (bestval > VTR)
                if (sum(oldBestval(length(oldBestval)-1002:end)-oldBestval(end))==0)
                    %newF=0.8;
                    %newCR=0.8;
                    itermax=iter;
                end
...
%     end
    ...
    %             t(numOfVariables-numOfconRefval+1:numOfVariables) = XVmin(numOfVariables-numOfconRefval+1:numOfVariables) + ...
    %                 rand(1,numOfconRefval).*(XVmax(numOfVariables-numOfconRefval+1:numOfVariables)...
    %                 - XVmin(numOfVariables-numOfconRefval+1:numOfVariables));
    %             t(numOfVariables-numOfconRefval+1:numOfVariables) = sort(t(numOfVariables-numOfconRefval+1:numOfVariables),'descend');
    %
    %         end
    %         ui(i,:)=t;
