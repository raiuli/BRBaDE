function [bestmem,bestval,nfeval] = BRBADEcustomeRWAWv4(fname,VTR,D,XVmin,XVmax,y,NP,itermax,F,CR,strategy,refresh)
...
val(1)  = feval(fname,pop(ibest,:),i);
bestval = val(1);                 % best objective function value so far
nfeval  = nfeval + 1;
for i=2:NP                        % check the remaining members
    val(i) = feval(fname,pop(i,:),i);
    nfeval  = nfeval + 1;
...
while ((iter < itermax) & (bestval > VTR))
    ...
    popHistory(:,:,iter)=pop;
        ...
        p = gcp();
        parfor i=1:NP
            f_history(i,iter) = feval(fname,pop(i,:),i);
        end
    %NP=10;
...
%         ffc=FC*10^n;
        ...
        %else
        [newF,newCR]=brbesFrDE(d11,d12, d21,d22);
