function [bestmem,bestval,nfeval] = BRBADEcustomeRWAWv2(fname,VTR,D,XVmin,XVmax,y,NP,itermax,F,CR,strategy,refresh)
load('dumpGlobalVariable.mat', 'observedOutput',...
...
s = strcat('Log/BRBinsDE_customeRWAW_',dateString,'.txt');
fid_x1 = fopen (s, 'w');
s = strcat('Log/bestvalue_customeRWAW_',dateString,'.txt');
fid_x2 = fopen (s, 'w');
s = strcat('Log/BRBinsDE_customeRWAW__IterationCount_',dateString,'.txt');
fid_x3 = fopen (s, 'w');
%-----Initialize population and some arrays-------------------------------

pop = zeros(NP,D); %initialize pop to gain speed
f_history_2=zeros(NP,itermax);
...
history_data=zeros(itermax,length(bestmem)+1);
...
%       parfor i=1:NP
%         f_history(i,iter) = feval(fname,pop(i,:),y);
%       end
%     for i=1:NP
%         f_history(i,iter) = feval(fname,pop(i,:),y);
%       end
...
%            %else
%               [newF,newCR]=brb_optimization_customeAWRW(d11,d12, d21,d22);
%     
%                %if newF~=0||newCR~=0
%                    F=newF;
%                    CR=newCR;
...
%     unstaisfiedOffspring=zeros(1,NP);
%     for i=1:NP
...
%             %fprintf ('\nAttribute Weight less then zero' );
%         end
%         if sum(c> 1) >0
%             constrainUnstaisfied=1;
%             %fprintf ('\nAttribute Weight greater then one' );
%         end
%         %fprintf ('\nRuleWeights\n');
...
%         if sum(c<0) >0
%             constrainUnstaisfied=1;
%             %fprintf ('\nRuleWeights less then zero' );
%         end
%         if sum(c>1) >0
%             constrainUnstaisfied=1;
%             %fprintf ('\nRuleWeights greater then one' );
%         end
%         %fprintf ('\nBelief Degrees\n');
%         z=t(numOfAttrWeight+numOfRuleWeight+1:numOfVariables-numOfconRefval);
%         j=1;
...
%             if sum(c<0) >0
%                 constrainUnstaisfied=1;
%                 %fprintf ('\nBelief Degrees less then zero\n' );
%             end
%             if sum(c>1) >0
%                 constrainUnstaisfied=1;
%                 %fprintf ('\nBelief Degrees greater then one\n' );
%             end
%             if sum(c)>1
%                 constrainUnstaisfied=1;
%                 %fprintf ('\nBelief Degrees sum greater then one\n' );
%             end
%             %fprintf ('\n');
...
%   %          end
%   %          t(numOfAttrWeight+numOfRuleWeight+1:numOfVariables-numOfconRefval)=z;
%             %tempval(i)=tempval(i)+ max_tempval;
%              t = XVmin + rand(1,D).*(XVmax - XVmin);
%             %unstaisfiedOffspring(i)=1;
%             constraintsFailedCount=constraintsFailedCount+1;
%         end
%        ui(i,:)=t;
%     end
    
%   fprintf ('\nTotal NP:%d constraintsFailedCount:%d constraintsSuccCount:%d \n',NP,constraintsFailedCount,(NP-constraintsFailedCount));
%   fprintf (fid_x1,'\nTotal NP:%d constraintsFailedCount:%d constraintsSuccCount:%d \n',NP,constraintsFailedCount,(NP-constraintsFailedCount));
  %  -----Select which vectors are allowed to enter the new population------------
%     parfor i=1:NP
%         tempval(i) = feval(fname,ui(i,:),y);   % check cost of competitor
% %         if unstaisfiedOffspring==1
% %             tempval(i) =tempval(i) +20;
% %         end    
%         nfeval  = nfeval + 1;
%     end
%    max_tempval=max(tempval);
    ...
    for i=1:NP
        ...
        if (tempval(i) <= val(i))  % if competitor is better than value in "cost array"
            ...
            f_history_2(i,iter+1)= tempval(i);
    ...
    %
    %     if(iter>1)
    %         history_data(iter,:)=horzcat(bestmem,bestval);
    %         p1 = history_data(iter-1,1:2);                         % First Point
    %         p2 = history_data(iter,1:2); ;                         % Second Point
    %         dp = p2-p1;                         % Difference
    %         %figure(1)
    ...
    %     else
    %         plot3(bestmem(1),bestmem(2),bestval,'dk','markersize',10);
    %         history_data(iter,:)=horzcat(bestmem,bestval);
    %     end
...
ss=sprintf('%s_brbinsde.mat',fname)
delete(ss)
save(ss,'data')
ss=sprintf('%s_brbinsde_his.mat',fname)
delete(ss)
save(ss,'history_data')
% fnamm='ass.mat'
